...
2. 7.7 minutes were needed to fit the 75 fixed loadings from step 1 to the random subset of tests. Now the first factor explains only 21% of the variance, and the PVE of the remaining factors is as follows.
...
3. The final backfitting (using the fixed loadings from step 1 and the priors on the factors from step 2) took 14.6 minutes. (Recall that I am using ash priors. Point-normal priors would likely require less time.) The first factor now explains 76% of the variance, with the PVE of the remaining factors as follows. (The moderately-sized PVE at index 61 corresponds to the one-hot vector for Pancreas).
...
Indeed, at both 5% and 1% significance levels, MASH and FLASH disagree in about 15-16% of cases. At 5%:
...
So, I will adjust for the relative conservativeness of FLASH by declaring effects significant according to FLASH when they have an LFSR $\le 0.05$ and significant according to MASH when they have an LFSR $\le 0.046$.  
...
ex_flnotm <- which(colSums(flnotm) >= 36)

legend_loc = c("topleft", "bottomleft", "topleft", "topleft", "bottomleft", "topleft")
...
There are five tests for which MASH declares significant 38 or more conditions that FLASH finds to be insignificant. I find these results to be more puzzling than the above results. It is clear that MASH is finding "identical effects" in the last three of these examples, and it is true that these are all cases in which the mean effect size is located away from zero. But it's not immediately clear that the latter is not simply the result of chance.

The other two examples (the first two) are fairly baffling. Here both MASH and FLASH find an effect that is shared among brain tissues, but MASH finds smaller (and uneven) significant effects in nearly all of the other conditions. I don't know what's happening here.

```{r mnotfl}
mnotfl <- !fl.signif & m.signif
ex_mnotfl <- which(colSums(mnotfl) >= 38)

legend_loc = c("topright", "topright", "topleft", "topright", "bottomright")
...
The following are all examples in which MASH and FLASH agree for all conditions. (There are many such examples --- I sampled six of them at random.)
