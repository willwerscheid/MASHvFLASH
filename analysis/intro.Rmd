...
so that, if the loadings $\ell_1, \ldots, \ell_k$ are orthogonal (in general, they are not),
$$ \begin{aligned}
\text{Cov} (X_{:, j}) 
...
`mashr` (see [here](https://stephenslab.github.io/mashr/index.html)) similarly models $X$ as a mixture of multivariate normals, so it makes sense to attempt to use `flashr` (which is, on its face, much simpler than `mashr`) to attain similar ends.
...
$$ Y = \begin{pmatrix} L & I_n \end{pmatrix} F'. $$
...
However, this is not always the case, and the two methods often perform similarly. See the simulation study [here](MASHvFLASHsims.html) and a comparison using GTEx data [here](MASHvFLASHgtex.html). 
